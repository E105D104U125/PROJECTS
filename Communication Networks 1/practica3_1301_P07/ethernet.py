...
ETH_FRAME_MIN = 60
#Tamaño cabecera de una trama Ethernet
ETH_HEADER_LEN = 14
PROMISC = 1
NO_PROMISC = 0
TO_MS = 10
...
def registerCallback(callback_func: Callable[[ctypes.c_void_p,pcap_pkthdr,bytes],None], ethertype:int) -> None:
    ...
    #Añadirá en la tabla (diccionario) de protocolos de nivel superior el dicha asociación.
...
def startEthernetLevel(interface:str) -> int:
    ...
    #Comprobar que la interfaz sea distinta de None.
    if (interface == None):
        return -1
    errbuf = bytearray()
...
def sendEthernetFrame(data:bytes,length:int,etherType:int,dstMac:bytes) -> int:
    ...
    #Si la trama tiene una longitud mayor de la máxima devuelve error.
    if ((ETH_HEADER_LEN + length) > ETH_FRAME_MAX):
    	print('A')
    	return -1

    #Construir la trama con la cabecera más el payload. Además, si la longitud es menor que la mínima, se completa la trama con 0's.
    buf = dstMac + macAddress + struct.pack('!H', etherType) +\
        (data + bytes([0x00]*(ETH_FRAME_MIN - ETH_HEADER_LEN - len(data))) if (ETH_HEADER_LEN + len(data) < ETH_FRAME_MIN) else data)

    #Llamar a pcap_inject para enviar la trama y comprobar el retorno de dicha llamada. En caso de que haya error notificarlo.
    if (pcap_inject(handle, buf, len(buf)) != len(buf)):
