...
/**
 * Libera la memoria de las colas, y borra sus nombres
 * 
 * @param client_queue cola de stream-client
 * @param server_queue cola de stream-server
 * */
...
/**
 * Libera la memoria de los semaforos, y borra sus nombres
 * 
 * @param sem_mutex semaforo de exlusion mutua
 * @param sem_empty semaforo n-ario
 * @param sem_fill semaforo n-ario
 * */
...
int main(int argc, char **argv){
    ...
    /* Se crean e inicializan los semaforos */
    ...
    while(cmp != 0){
        ...
        if (strcmp(msg, "post") == 0){
            if (mq_send(server_queue, (char *)msg, sizeof(msg), 1) == -1){
                fprintf(stderr, "post: Error en el envio del mensaje\n");
            }
        }        
        /* Envio del mensaje al cliente */
        else if (strcmp(msg, "get") == 0){
            if (mq_send(client_queue, (char *)msg, sizeof(msg), 1) == -1){
                fprintf(stderr, "get: Error en el envio del mensaje\n");
            }
        }
        /* Mensaje de finalizacion a ambos procesos */
        else if (cmp == 0){
            if (mq_send(server_queue, (char *)msg, sizeof(msg), 1) == -1){
                fprintf(stderr, "exit: Error en el envio del mensaje\n");
                break;
            }
            if (mq_send(client_queue, (char *)msg, sizeof(msg), 1) == -1){
                fprintf(stderr, "exit: Error en el envio del mensaje\n");
